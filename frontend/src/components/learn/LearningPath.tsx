        ...
        <div key={unit.id} className="mb-12">
          ...
          <div className="flex flex-col items-center gap-6 py-4 relative">
            {unit.skills.map((skill, index) => {
              ...
              const isStartNode = index === 0 && skill.status === 'active';
              ...
              return (
                ...
                >
                  {/* Floating START tooltip matching Figma */}
                  {isStartNode && (
                    <div className="absolute -top-12 bg-[#58cc02] text-white font-extrabold text-xs px-3.5 py-1.5 rounded-xl uppercase tracking-widest shadow-md border-2 border-white animate-bounce z-10 flex items-center gap-1">
                      <span>START</span>
                      <div className="absolute left-1/2 -bottom-1.5 -translate-x-1/2 w-3 h-3 bg-[#58cc02] rotate-45" />
                    </div>
                  )}
                  ...
                  {skill.icon === 'chest' ? (
                    ...
                    /* Skill Node Button */
                    <Link
                      href={skill.status !== 'locked' ? `/lesson/${skill.id}` : '#'}
